...
test.describe.serial('Pago mixto (pago dividido)', () => {
  test('POS: venta mixta → 2 filas payments; el efectivo entra a caja, el nequi no', async ({ page }) => {
    ...
    await expect(page.getByTestId('checkout-confirm-mixto')).toBeEnabled()
  ...
  test('POS: validación bloqueante — Σ≠total deshabilita Cobrar (falta y excedido)', async ({ page }) => {
    ...
    const confirm = page.getByTestId('checkout-confirm-mixto')
  ...
  test('POS: venta simple (un método) sigue funcionando — 1 fila payment', async ({ page }) => {
    ...
    // 🔴 Sitio decidido aparte de los otros cuatro de este archivo: acá NO hay
    //    reparto, así que el botón que cobra es el primario del paso de método
    //    —`checkout-continue`—, no el confirmador del split. En la columna los
    //    dos eran `cobro-confirmar` y esta diferencia no se veía.
