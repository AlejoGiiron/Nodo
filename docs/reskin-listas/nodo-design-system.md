...
|---|---|
| **Mostrador** | `error de validación` · `cliente sin cupo` · **`venta excede el cupo`** · `cliente en mora` · **`cliente sin lista`** · **`nivel sin precio`** |
...
| **Catálogo** | `error de validación` · **`nivel vacío`** |
| **Inventario** | **`producto sin costo`** |
| **Clientes** | `cliente sin cupo` · `cliente en mora` · **`cliente sin lista`** |
...
18. **Qué pasa con la lista del cliente cuando cambia el precio del producto.** Las listas son números tecleados, no fórmulas; si L1 sube, L2–L4 no se mueven solas. Si debe existir un aviso de "niveles desalineados" no está decidido.
19. **A qué nivel se vende a un cliente sin lista.** El diseño asume L1 y lo dice en pantalla, pero es una regla de negocio que la dueña tiene que confirmar.
20. **L0.** Existe, está vacía y se parametriza después. Qué significa (¿precio de costo? ¿promoción? ¿interno?) no está decidido, y el nombre que reciba depende de eso.
