...
| 22 | **Tasa de ruido del hook, sin medir.** 6 disparos en las dos primeras sesiones (2026-08-31), **los 6 falsos positivos**, en sesiones que no escribieron una sola línea de SQL. ⛔ **No corregir todavía — primero medir**, separando las dos clases (abajo). ⛔ **El instrumento NO existe todavía y el conteo a mano no lo es:** en la sesión del inventario el hook disparó del orden de 20 veces y perdí la cuenta exacta a mitad de camino. Una tasa medida de memoria no sirve para decidir sobre el hook. ✅ **RESUELTO el 2026-08-31: el hook ya lo apendea solo** a `.claude/hook-ledger.jsonl` (fecha, herramienta, qué regla matcheó), fuera de git. Verificado en esta máquina tras el cambio: `node --check` limpio, sin bytes de control, 811 bytes en los 4 casos que deben disparar —ruta Windows incluida—, 2597 en el de catálogo, 0 en los que deben callar. **Divergencia con G-Vento aceptada y registrada** en R1 punto 9: instrumentar puede divergir, corregir no. ✅ **El denominador también está** (`hook-ledger.stats.json`): contador agregado de `invocaciones` y `disparos`, más el desglose por regla. Agregado como contador y no como una línea por invocación para que el archivo no crezca sin control. ⚠️ **Leerlo con el sesgo puesto:** es read-modify-write y no es atómico, así que `invocaciones` es un **piso**; subestimar el denominador **infla** la tasa, o sea que un número alto es real y uno bajo hay que desconfiarlo. **Ahora sí se puede decidir sobre el hook con datos** — falta acumularlos en sesiones que escriban SQL de verdad, que es donde puede haber verdaderos positivos. | 🔴 **6 de 6 NO es una tasa.** El denominador es "sesiones que no escribieron SQL": ahí el 100% de falsos positivos es el resultado *esperado*, no una anomalía. La tasa real aparece cuando existan sesiones que **sí** escriban SQL, porque solo ahí hay verdaderos positivos posibles. Citar "6 de 6" como tasa de ruido sería el error de proxy de R4 sobre nuestra propia medición. |
| 22.A | **Clase A — nombrar una ruta `.sql`** (5 de 6). Es la **regla 1** funcionando como fue diseñada: matchea la ruta, no distingue prosa de código. | Ruido de bajo costo conceptual: se dispara al *hablar de* un archivo. Si el arreglo alguna vez llega, acá tendría la forma de mirar el rol del token (¿va como destino de escritura, o dentro de texto?), no la de una lista de excepciones. |
| 22.B | **Clase B — escribir sobre el catálogo sin tocarlo** (1 de 6: un mensaje de commit que mencionaba `SYSTEM_ROLES`). Es la **regla 2** matcheando **prosa**. | 🔴 **Propiedad incómoda, y es la que cambia hacia dónde iría el arreglo: cuanto mejor se documenta el trabajo sobre RBAC, más dispara.** Penaliza la conducta correcta — un commit que explica bien un cambio de permisos es más ruidoso que uno que no explica nada. Eso es un incentivo invertido, no solo ruido: presiona hacia mensajes de commit pobres, que es exactamente lo que el proyecto no quiere. El arreglo de la clase B **no** es el mismo que el de la A: acá el problema no es el matcheo de rutas sino que la regla 2 no distingue *modificar* el catálogo de *hablar* del catálogo. |
