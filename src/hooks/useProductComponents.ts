...
export function useProductComponents(parentId: string | null) {
  ...
  return {
     ...
     * 🔴 Y EL `!parentId` NO ES DEFENSIVO: ES EL CASO DEL PRODUCTO NUEVO, y
     *    olvidarlo rompio la creacion. La consulta lleva `enabled: !!parentId`,
     ...
     */
    datoConfirmado: !parentId || (!query.isPending && !query.isFetching),
    reconcile,
  }
}
